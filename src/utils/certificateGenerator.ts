...
export function generatePassportCanvas(data: PassportData): Promise<HTMLCanvasElement> {
  return new Promise((resolve, reject) => {
    try {
      ...
      ctx.fillText('PASSEPORT DARIJA', canvas.width / 2, 140);
      
      ctx.fillStyle = '#94a3b8'; // slate-400
      ctx.font = '32px sans-serif';
      ctx.fillText('جواز سفر الدارجة', canvas.width / 2, 190);
      ...
      ctx.fillText(`ID: ${data.passportId}`, 120, canvas.height - 80);
      
      ctx.textAlign = 'right';
      ctx.fillText(`Délivré le: ${data.date}`, canvas.width - 120, canvas.height - 80);

      // Stamp-like effect "VALIDÉ"
      ctx.save();
      ctx.translate(canvas.width / 2 + 100, canvas.height - 180);
