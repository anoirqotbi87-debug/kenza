  ...
  if (sessionCards.length === 0) {
    return (
      <div className="fixed inset-0 bg-white z-50 flex flex-col items-center justify-center p-4">
        ...
        <h2 className="text-3xl font-bold text-slate-800 mb-4">Tout est à jour !</h2>
        <p className="text-slate-600 mb-8 max-w-md text-center">
          Vous n'avez aucune carte à réviser aujourd'hui. Revenez demain pour consolider votre mémoire.
        </p>
  ...
  if (sessionComplete) {
    return (
      <div className="fixed inset-0 bg-white z-50 flex flex-col items-center justify-center p-4">
        ...
        <h2 className="text-3xl font-bold text-slate-800 mb-4">Session Terminée</h2>
        <p className="text-slate-600 mb-8 max-w-md text-center">
          Vous avez révisé {sessionCards.length} mots avec succès. +{sessionCards.length * 5} XP gagnés !
        </p>
  ...
  return (
    <div className="fixed inset-0 bg-slate-900 z-50 flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        >
          {!isFlipped ? (
            <div className="flex flex-col items-center justify-center w-full h-full" style={{ backfaceVisibility: 'hidden' }}>
              <div className="text-sm font-bold text-slate-400 mb-4 uppercase tracking-wide">Traduis en {preferredNotation}</div>
              <div className="text-3xl font-bold text-slate-800">
                Word_{currentCard.wordId} {/* In a real app, fetch word translation from dictionary */}
              </div>
            ...
            <div className="flex flex-col items-center justify-center w-full h-full" style={{ backfaceVisibility: 'hidden', transform: 'rotateY(180deg)' }}>
               <div className="text-sm font-bold text-green-500 mb-4 uppercase tracking-wide">Réponse</div>
               <div className="text-4xl font-extrabold text-slate-800 mb-2">
                 Translation_{currentCard.wordId}
               </div>
            </div>
          )}
        </div>
        ...
        {isFlipped && (
          <div className="grid grid-cols-4 gap-2 mt-8 animate-in slide-in-from-bottom-4">
